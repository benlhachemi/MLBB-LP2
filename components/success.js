...
const Success = ({msg,question,yes,no,setSuccess, setStep_1, setStep_2,actions}) => {
    return (
            ...
            <center>
                ...
                <div className="mt-4 w-full pb-4 bg-white text-center bg-opacity-40 rounded-md shadow-lg justify-center text-white">
                    ...
                    <div className="flex py-3 px-10 flex-1 text-center justify-center align-middle">
                        <div onClick={e => {actions.addBtn('yes confirm userId');setSuccess(false);setStep_1(false);setStep_2(true)}} className="cursor-pointer border-2 mx-2 py-2 px-5 rounded-lg bg-green-600">
                            <CheckIcon />
                            <div>{yes}</div>
                        </div>
                        <div onClick={e => {actions.addErr('Not the userId button');setSuccess(false)}} className="cursor-pointer border-2 mx-2 py-2 px-5 rounded-lg bg-red-600">
                            <ClearIcon />
                            <div>{no}</div>
                        </div>
                    </div>
